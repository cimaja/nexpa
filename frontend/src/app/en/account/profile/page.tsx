...
export default function ProfilePage() {
  ...
  const validateForm = () => {
    const newErrors = {
      firstName: form.firstName ? '' : t('profile.errors.firstName', 'First name is required'),
      lastName: form.lastName ? '' : t('profile.errors.lastName', 'Last name is required'),
      email: !form.email ? t('profile.errors.email', 'Email is required') : 
             !/\S+@\S+\.\S+/.test(form.email) ? t('profile.errors.emailInvalid', 'Invalid email format') : '',
      phone: '',
      general: ''
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({
        ...errors,
        general: t('profile.errors.general', 'An error occurred while updating your profile. Please try again.')
      });
      setSaving(false);
    }
  };
        ...
        <div className="md:col-span-3">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-semibold">
                {t('profile.title', 'Edit Profile')}
              </h2>
            </div>
            
            {success && (
              <div className="mb-6 p-3 bg-green-100 text-green-700 rounded">
                {t('profile.success', 'Your profile has been updated successfully.')}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="border-t border-gray-200 dark:border-slate-700 pt-6 flex justify-between">
                <Link
                  href="/en/account/change-password"
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6 mt-6">
            ...
            <p className="mb-4 text-gray-600 dark:text-gray-400">
              {t('profile.deleteAccountWarning', 'Deleting your account is a permanent action and cannot be undone. All of your data will be permanently removed.')}
            </p>
            
            <button
              onClick={() => {
                if (confirm(t('profile.deleteConfirm', 'Are you sure you want to delete your account? This action cannot be undone.'))) {
